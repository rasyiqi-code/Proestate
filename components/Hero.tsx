...
const Hero = ({ content }: { content: any }) => {
  return (
    <section className="relative bg-pro-dark-blue text-white z-10" style={{ backgroundImage: `url(${content?.imageUrl || 'https://picsum.photos/1920/1080?grayscale&blur=2'})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <div className="relative container mx-auto px-6 py-24 lg:py-32 flex flex-col lg:flex-row items-center">
        <div className="lg:w-1/2 text-center lg:text-left z-10">
          <span className="inline-block bg-white/10 border border-white/20 text-white px-4 py-1 rounded-full text-sm font-medium mb-4">
            {content?.badge || 'Trusted & Secure Property Solutions'}
          </span>
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-4">
            {content?.title || 'Your Trusted Property Partner'}
          </h1>
          <p className="text-lg text-gray-300 mb-8">
            {content?.subtitle || 'Pro Estate specializes in transparent, secure real estate transactions with cutting-edge digital solutions. We serve global clients with professional expertise and multilingual support.'}
          </p>
          <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 justify-center lg:justify-start">
            <button className="bg-pro-yellow text-pro-dark-blue font-bold py-3 px-6 rounded-lg flex items-center justify-center space-x-2 hover:brightness-110 transition-transform transform hover:scale-105">
              <span>{content?.primaryButtonText || 'Explore Properties'}</span>
        ...
        <div className="lg:w-1/2 mt-12 lg:mt-0 lg:pl-16 z-10">
          <div className="bg-white/5 backdrop-blur-sm p-4 rounded-2xl shadow-lg relative">
            <img src={content?.showcaseImageUrl || 'https://picsum.photos/800/600'} alt="Modern House" className="rounded-xl w-full" />
            <div className="absolute top-8 right-8 bg-pro-yellow text-pro-dark-blue px-4 py-3 rounded-lg text-center shadow-xl">
              <p className="font-bold text-lg">{content?.imageBadge?.value || '1000+'}</p>
              <p className="text-sm">{content?.imageBadge?.label || 'Global Clients'}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {content?.features?.map((feature: any, index: number) => (
               <div key={index} className="flex flex-col items-center">
                {feature.icon === 'Globe' && <GlobeAltIcon className="w-8 h-8 text-pro-yellow mb-2" />}
                {feature.icon === 'Shield' && <ShieldCheckIcon className="w-8 h-8 text-pro-yellow mb-2" />}
                {feature.icon === 'Device' && <DevicePhoneMobileIcon className="w-8 h-8 text-pro-yellow mb-2" />}
